        ...
        if not use_sdk or generated_text is None:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if not response or not response.is_success:
                    error_detail = last_error if 'last_error' in locals() else "No available models found. Please check your API key."
                ...
                try:
                    ...
                    raise HTTPException(
                        status_code=500,
                        detail=f"Gemini API returned non-JSON response: {safe_preview}"
                    )
